...
export default function Exercices({ Exercice }) {
    ...
    useEffect(() => {
        ...
        if (ratioMuscu.Strength[Exercice].lest === '1') {

            const exo = firebase.database().ref(`users/` + currentUser.uid + "/muscu/stats/" + ratioMuscu.Strength[Exercice].name.En + '/10')
        ...
        } else if (ratioMuscu.Strength[Exercice].lest === '2') {
            dropRef = '1'
            rep = [1]
            setRepSet('set')
            setUnit('repetitions')
            setdropdowTitle('1 set')
            const exo = firebase.database().ref(`users/` + currentUser.uid + "/muscu/stats/" + ratioMuscu.Strength[Exercice].name.En + '/1')
            exo.get().then((snapshot) => {
                let previousList = snapshot.val()
                if (previousList !== undefined && previousList !== null) {
                    setExerciceData(previousList)
    ...
    const submit = () => {
        if (parseInt(useData.current.value) > 0 && parseInt(useData.current.value) < 2000) {
            ...
            const exo = firebase.database().ref(`users/` + currentUser.uid + "/muscu/stats/" + ratioMuscu.Strength[Exercice].name.En + "/" + dropRef)
            exo.get().then((snapshot) => {
                let previousList = snapshot.val()
                if (previousList !== undefined && previousList !== null) {
    ...
    const ratioProcess = (data, rep) => {
        ...
        userData.get().then((snapshot) => {
            ...
            let ageFactor
            if (previousList.age <= 17) ageFactor = 0.87
            if (previousList.age >= 18 && previousList.age <= 23) ageFactor = 0.98
            if (previousList.age >= 24 && previousList.age <= 39) ageFactor = 1
            if (previousList.age >= 40 && previousList.age <= 49) ageFactor = 95
            if (previousList.age >= 50 && previousList.age <= 59) ageFactor = 0.83
            if (previousList.age >= 60 && previousList.age <= 69) ageFactor = 0.79
            if (previousList.age >= 70 && previousList.age <= 79) ageFactor = 0.55
            if (previousList.age >= 89) ageFactor = 0.44
            let repFactor
            ...
            newRatio = (((data / previousList.weight) * 100) / (userSex?.trophyGold * ageFactor)) * 100 / repFactor
            ...
            oldratio.get().then((snapshot) => {
                ...
                if (token === true) {
                    const exerciceTrophy = firebase.database().ref(`users/` + currentUser.uid + "/muscu/stats/" + ratioMuscu.Strength[Exercice].name.En + "/trophy")
                    exerciceTrophy.get().then((snapshot) => {
                        ...
                        trophy.get().then((snapshot) => {
                            ...
                            if (previousList3.medalBronze === undefined && newRatio >= ((userSex?.medalBronze * 100) / userSex?.trophyGold)) {
                                previousList3.medalBronze = 1
                                exerciceTrophy.set(previousList3)
                                let bronzeList = trophyList
                                console.log(bronzeList, 'bronzeList')
                                bronzeList.medalBronze += 1
                                trophy.set(bronzeList)

                            }
                            if (previousList3.medalSilver === undefined && newRatio >= ((userSex?.medalSilver * 100) / userSex?.trophyGold)) {
                                previousList3.medalSilver = 1
                                exerciceTrophy.set(previousList3)
                                let SilverList = trophyList
                                SilverList.medalSilver += 1
                                trophy.set(SilverList)

                            }
                            if (previousList3.medalGold === undefined && newRatio >= ((userSex?.medalGold * 100) / userSex?.trophyGold)) {
                                previousList3.medalGold = 1
                                exerciceTrophy.set(previousList3)
                                let GoldList = trophyList
                                GoldList.medalGold += 1
                                trophy.set(GoldList)

                            }
                            if (previousList3.trophyBronze === undefined && newRatio >= ((userSex?.trophyBronze * 100) / userSex?.trophyGold)) {
                                previousList3.trophyBronze = 1
                                exerciceTrophy.set(previousList3)
                                let bronzeList = trophyList
                                bronzeList.trophyBronze += 1
                                trophy.set(bronzeList)

                            }
                            if (previousList3.trophySilver === undefined && newRatio >= ((userSex?.trophySilver * 100) / userSex?.trophyGold)) {
                                previousList3.trophySilver = 1
                                exerciceTrophy.set(previousList3)
                                let SilverList = trophyList
                                SilverList.trophySilver += 1
                                trophy.set(SilverList)

                            }
                            if (previousList3.trophyGold === undefined && newRatio >= 100) {
                                previousList3.trophyGold = 1
                                exerciceTrophy.set(previousList3)
                                let GoldList = trophyList
                                GoldList.trophyGold += 1
                                trophy.set(GoldList)
                            }


                            if (previousList3.medalBronze === 1 && newRatio < ((userSex?.medalBronze * 100) / userSex?.trophyGold)) {
                                previousList3.medalBronze = null
                                exerciceTrophy.set(previousList3)
                                let bronzeList = trophyList
                                bronzeList.medalBronze -= 1
                                trophy.set(bronzeList)
                            }
                            if (previousList3.medalSilver === 1 && newRatio < ((userSex?.medalSilver * 100) / userSex?.trophyGold)) {
                                previousList3.medalSilver = null
                                exerciceTrophy.set(previousList3)
                                let SilverList = trophyList
                                SilverList.medalSilver -= 1
                                trophy.set(SilverList)
                            }
                            if (previousList3.medalGold === 1 && newRatio < ((userSex?.medalGold * 100) / userSex?.trophyGold)) {
                                previousList3.medalGold = null
                                exerciceTrophy.set(previousList3)
                                let GoldList = trophyList
                                GoldList.medalGold -= 1
                                trophy.set(GoldList)
                            }
                            if (previousList3.trophyBronze === 1 && newRatio < ((userSex?.trophyBronze * 100) / userSex?.trophyGold)) {
                                previousList3.trophyBronze = null
                                exerciceTrophy.set(previousList3)
                                let bronzeList = trophyList
                                bronzeList.trophyBronze -= 1
                                trophy.set(bronzeList)
                            }
                            if (previousList3.trophySilver === 1 && newRatio < ((userSex?.trophySilver * 100) / userSex?.trophyGold)) {
                                previousList3.trophySilver = null
                                exerciceTrophy.set(previousList3)
                                let SilverList = trophyList
                                SilverList.trophySilver -= 1
                                trophy.set(SilverList)
                            }
                            if (previousList3.trophyGold === 1 && newRatio < 100) {
                                previousList3.trophyGold = null
                                exerciceTrophy.set(previousList3)
                                let GoldList = trophyList
                                GoldList.trophyGold -= 1
                                trophy.set(GoldList)
    ...
    const selectRep = (e) => {
        ...
        const exo = firebase.database().ref(`users/` + currentUser.uid + "/muscu/stats/" + ratioMuscu.Strength[Exercice].name.En + "/" + e.nativeEvent.target.id)
        exo.get().then((snapshot) => {
            let previousList = snapshot.val()
            setExerciceData(previousList)
            setRender(render +1)
    ...
    const remove = async () => {
        ...
        oldratio.get().then((snapshot) => {
            ...
            exo.get().then((snapshot) => {
                ...
                const fullexo = firebase.database().ref(`users/` + currentUser.uid + "/muscu/stats/" + ratioMuscu.Strength[Exercice].name.En)
    ...
    return (
        <div className="mt-3">
            ...
            <h2 className="text-center">{ratioMuscu.Strength[Exercice]?.name.Fr}</h2>
            <Form className="mt-3" >
                <Form.Group id="name">
